...
- To override a file baked into the RAGFlow image (e.g. a prompt in `rag/prompts/*.md`) without building a custom image: put the replacement under `helm/overrides/` (e.g. `helm/overrides/prompts/` for prompt files; must be inside the chart dir — `.Files.Glob`/`.Files.Get` can't read outside it), load it into a ConfigMap (`helm/templates/vision-prompts-config.yaml` is the template for this), and `subPath`-mount it onto the in-container path (RAGFlow lives at `/ragflow/`, so e.g. `/ragflow/rag/prompts/<name>.md`). Mount only on the deployment that uses it — figure/image description runs in the **taskexecutor** (parsing), not the webserver. Add a `checksum/<name>` pod annotation so the pod rolls on change; RAGFlow caches prompt files in-process (`rag/prompts/template.py` `_loaded_prompts`), so a restart is required to pick up edits. Use a plain `subPath` per file, never a directory mount, which would hide the rest of the package.
