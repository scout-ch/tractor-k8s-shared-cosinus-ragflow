...
Write the entire description in the same language as the surrounding document context and the text visible in the image. Do not translate; match the document's own language, which is most likely German, French, or Italian. If the language is genuinely ambiguous, infer it from the surrounding context or any visible text — do not default to English.
